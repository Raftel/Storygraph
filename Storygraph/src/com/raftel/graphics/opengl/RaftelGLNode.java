...
public class RaftelGLNode {
	...
	private RaftelGLNode mParent = null;
	private ArrayList<RaftelGLNode> mChildList = new ArrayList<RaftelGLNode>();

	public static class PickedNode {
		public RaftelGLNode mNode = null;
	...
	public RaftelGLNode getParent() {
		return mParent;
	}

	public void setParent(RaftelGLNode parent) {
		if (mParent != parent) {
			mParent = parent;
			setMatrixDirty();
		}
	...
	public ArrayList<RaftelGLNode> getChildList() {
		return mChildList;
	}

	public boolean addChild(RaftelGLNode node) {
		if (node == null)
			return false;

		RaftelGLNode prevParent = node.getParent();
		if (prevParent == this)
			return true;
	...
	public boolean removeChild(RaftelGLNode node) {
		if (node == null)
			return false;
	...
	public void setMatrixDirty() {
		...
		for (int i = 0; i < mChildList.size(); i++) {
			RaftelGLNode node = mChildList.get(i);
			if (node != null)
				node.setMatrixDirty();
		}
